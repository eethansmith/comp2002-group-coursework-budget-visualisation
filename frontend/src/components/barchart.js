...
const BarChart = (props) => {
    ...
    const NUM_BARS = Object.keys(props.data).length;

    // These two variables determine where along the axes the bars will 
    const AXES_START = HEIGHTWIDTH/10 + (HEIGHTWIDTH/20)/2;
    const AXES_END = (9*HEIGHTWIDTH)/10 - (HEIGHTWIDTH/20)/2;

    const TOTAL_AXES_LENGTH = AXES_END - AXES_START;

    // Pre-decided bar width and spacing based on axes width
    const BAR_SPACING_UTIL = TOTAL_AXES_LENGTH/(NUM_BARS);

    const BAR_WIDTH = BAR_SPACING_UTIL*7/8;
    ...
    Object.keys(props.data).forEach((key, index) => {
        bars.push(
            <rect 
                ...
                x={
                    (BAR_SPACING_UTIL*(index + 1/16)) + AXES_START
                } 
        ...
        xLabels.push(
            <text
                key={key}
                x={(HEIGHTWIDTH/NUM_BARS)*index + (HEIGHTWIDTH/NUM_BARS)/2}
...
function getLines(height, data) {
    ...
    const MULTIPLIER = height / MAX_VALUE;
    const lines = [];
    var yLabelValues = [];

    yLabelValues = pushLines(MAX_VALUE);
