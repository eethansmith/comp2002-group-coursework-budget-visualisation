...
test('Renders Daily view ringchart', async () => {
  const {container} = render(<RingChart data={{}} isDaily={true} />);
  const textElement = screen.getByText(/Today/i);
  expect(textElement).toBeInTheDocument();
});

test('Renders Monthly view ringchart', async () => {
  const {container} = render(<RingChart data={{}} isDaily={false} />);
...
test('Hover feature', () => {
  const {container} = render(<RingChart data = {{Test: 32, Other: 45}} isDaily={false}/>);
  fireEvent.mouseEnter(container.firstChild.childNodes[0]);
  const textElement = screen.getByText(/Test/i);
  expect(textElement).toBeInTheDocument();
});
...
test('Hover feature 2', () => {
  const {container} = render(<RingChart data = {{Test: 32, Other: 45}} isDaily={false}/>);
  fireEvent.mouseDown(container.firstChild.childNodes[1]);
  const textElement = screen.getByText(/Other/i);
  expect(textElement).toBeInTheDocument();
});